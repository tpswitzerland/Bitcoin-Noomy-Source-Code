...
*Setup instructions for a Gitian build of BitcoinNoomy Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitcoinNoomy
...
- [Building BitcoinNoomy Core](#building-bitcoinnoomy-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitcoinnoomy-host/README.md).
...
Clone the git repositories for bitcoinnoomy and Gitian.
...
in the bitcoinnoomy repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building BitcoinNoomy Core
----------------

To build BitcoinNoomy Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitcoinnoomy repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinnoomy/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitcoinnoomy/bitcoinnoomy
...
For example:
```bash
URL=https://github.com/laanwj/bitcoinnoomy.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoinnoomy=${COMMIT} --url bitcoinnoomy=${URL} ../bitcoinnoomy/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinnoomy=${COMMIT} --url bitcoinnoomy=${URL} ../bitcoinnoomy/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinnoomy=${COMMIT} --url bitcoinnoomy=${URL} ../bitcoinnoomy/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitcoinnoomy git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcoinnoomy/contrib/gitian-descriptors/*|sort|uniq )
...
BNYPATH=/some/root/path/bitcoinnoomy
SIGPATH=/some/root/path/bitcoinnoomy-detached-sigs

./bin/gbuild --url bitcoinnoomy=${BNYPATH},signature=${SIGPATH} ../bitcoinnoomy/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinnoomy-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinnoomy-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoinnoomy-osx-build.assert
```
...
[bitcoinnoomy-core/gitian.sigs](https://github.com/bitcoinnoomy-core/gitian.sigs/) repository, or if that's not possible create a pull
