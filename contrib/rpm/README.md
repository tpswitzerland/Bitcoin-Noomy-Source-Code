...
The RPM spec file provided here is for BitcoinNoomy-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinnoomy.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitcoinNoomy git repository](https://github.com/bitcoinnoomy/bitcoinnoomy) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinnoomy.conf` file.
...
in the bitcoinnoomy git repository. At present only an SVG version of the BitcoinNoomy
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinnoomy
...
needed when 0.12.1 is released, a proper fix is already in the BitcoinNoomy git
master branch.
...
idea to build BitcoinNoomy Core against a static version of that library compiled
...
The `bitcoinnoomy.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinnoomy-server` package. At this point, most RPM based
...
The `bitcoinnoomy-server` package should have SELinux support. How to properly do
...
BitcoinNoomy-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinnoomy/). He has been
packaging BitcoinNoomy for Fedora at least since 2012.
...
4. I renamed the `bitcoinnoomy` package that contains the Qt GUI to `bitcoinnoomy-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinnoomy-xt` or `bitcoinnoomy-classic`. I wanted to make sure the general population
