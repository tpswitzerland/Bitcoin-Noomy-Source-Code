...
###   bitcoinnoomycore.org and bitcoinnoomy.org and compares them.
...
WORKINGDIR="/tmp/bitcoinnoomy_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinnoomycore.org"
HOST2="https://bitcoinnoomy.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinnoomy-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinnoomy-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinnoomy.org failed to provide signature file, but bitcoinnoomycore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinnoomy.org and bitcoinnoomycore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinNoomy Core binary release signing key installed?"
   fi
