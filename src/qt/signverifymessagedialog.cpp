...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CBitcoinNoomyAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid()) {
...
{
    CBitcoinNoomyAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid()) {
    ...
    if (!(CBitcoinNoomyAddress(pubkey.GetID()) == addr)) {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
